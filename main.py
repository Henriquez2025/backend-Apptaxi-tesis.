...
# Construcción URL Pooler (Puerto 6543 - IPv4)
...
class Usuario(Base):
    ...
    email = Column(String, unique=True) # Ya no tiene nombre aquí
    password_hash = Column(String) 
    role = Column(String) 
    
    # Relaciones inversas (Para acceder al perfil desde el usuario)
...
class Administrador(Base): # Nueva tabla
...
class Emergencia(Base): # Antes contacto_emergencia
...
class UsuarioRegistroRequest(BaseModel):
    nombre: str # Recibe nombre completo desde Flutter
...
class UsuarioAdmin(ModelView, model=Usuario):
    ...
    column_list = [Usuario.id, Usuario.email, Usuario.role] # Quitamos nombre, ya no existe aquí

class ClienteAdmin(ModelView, model=Cliente):
    name, name_plural, icon = "Cliente", "Clientes", "fa-solid fa-person"
    column_list = [Cliente.id, Cliente.nom_apell, Cliente.ciudad, Cliente.telefono]

class ConductorAdmin(ModelView, model=Conductor):
    name, name_plural, icon = "Conductor", "Conductores", "fa-solid fa-id-card"
    column_list = [Conductor.id, Conductor.nom_apell, "vehiculo.placa", Conductor.telefono]
...
# 5. ENDPOINTS ACTUALIZADOS (Lógica nom_apell)
...
async def login(datos: LoginRequest, db: AsyncSession = Depends(get_db)):
    try:
        # 1. Autenticar en tabla USUARIOS
        ...
        # 2. Buscar NOMBRE real en la tabla de perfil correspondiente
        # Como 'usuarios' ya no tiene nombre, lo buscamos en clientes, conductores o administradores
        nombre_real = "Usuario"
...
async def registrar_usuario(datos: UsuarioRegistroRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        async with db.begin():
            ...
            # 1. Crear Usuario (Solo Email y Pass)
...
async def registrar_conductor(datos: RegistroConductorRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        async with db.begin():
            ...
            # 3. Conductor (Con nom_apell)
            f_nac = datetime.strptime(datos.fecha_nacimiento, "%Y-%m-%d").date() if datos.fecha_nacimiento else None
            await db.execute(text("INSERT INTO conductores (usuario_id, vehiculo_id, nom_apell, telefono, fecha_nacimiento) VALUES (:u, :v, :n, :t, :f)"), 
...
async def agregar_contacto(d: ContactoRequest, db: AsyncSession = Depends(get_db)):
    try:
        async with db.begin():
            # Apunta a la nueva tabla 'emergencia'
