...
import random # Agregamos random para generar la clave
...
# Schema nuevo para actualizar estado de viaje
class EstadoViajeRequest(BaseModel):
    viaje_id: int
    nuevo_estado: str

# Schema para cancelar viaje
...
def leer_raiz(): 
    ...
    return {"mensaje": "API Taxi Funcionando (v14.0 - Ciclo de Viaje)."}
        ...
        async with db.begin():
            ...
            query = text(f"""
                INSERT INTO viajes (
                    ...
                    origen_geom, destino_geom, clave_seguridad
                ) VALUES (
                    :cid, :ori, :des, :tar, 'pendiente', 
                    :olat, :olng, :dlat, :dlng,
                    {geo_ori}, {geo_des}, :clave
                )
                RETURNING id
            """)
...
async def ver_pendientes(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        """)
        res = await db.execute(query)
        return [{"id": v.id, "origen": v.origen, "destino": v.destino, "tarifa": v.tarifa, "estado": v.estado, "cliente": v.nom_apell or "Cliente", "origen_lat": v.origen_lat, "origen_lng": v.origen_lng, "destino_lat": v.destino_lat, "destino_lng": v.destino_lng} for v in res.fetchall()]
    except: return []
...
# --- NUEVO: ENDPOINT PARA CAMBIAR ESTADO DE VIAJE (EN CURSO / FINALIZADO) ---
...
async def cancelar_viaje(d: CancelarViajeRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        async with db.begin():
            ...
            # Reglas de negocio: No cancelar si ya arrancó
            if estado_actual in ['en_curso', 'finalizado', 'cancelado']:
                return {"error": "El viaje ya está en curso o finalizado, no se puede cancelar."}
...
async def obtener_viaje(viaje_id: int, db: AsyncSession = Depends(get_db)):
    """Consulta estado del viaje en tiempo real."""
    try:
        query = text("""
...
async def actualizar_ubicacion(datos: UbicacionConductorRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        async with db.begin():
            # Actualizamos lat/lng y también la columna geométrica para PostGIS
