...
# Configuración de conexión a Supabase (Pooler IPv4)
...
# Selección de URL de base de datos
...
# Palabras clave para seguridad de viaje
PALABRAS_CLAVE = [
...
# Eliminar usuario en Supabase Auth (admin)
...
@app.get("/")
def leer_raiz():
    return {"mensaje": "API Taxi Running (v28.0 - Transaction Fix)."}
...
async def registrar_conductor(datos: RegistroConductorRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        vid = (await db.execute(
            ...
            {"ma": datos.vehiculo_marca, "mo": datos.vehiculo_modelo, "pl": datos.vehiculo_placa,
             "co": datos.vehiculo_color, "an": datos.vehiculo_anio},
        )).scalar()
...
# Registro de conductor con Supabase Auth (endpoint API)
